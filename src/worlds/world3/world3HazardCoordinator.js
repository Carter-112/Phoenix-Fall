...
export class World3HazardCoordinator {
    ...
    spawnHazard(x, y) {
        ...
        // Determine which type of hazard to spawn - Only ice-themed hazards for World 3
        const random = Math.random();
        
        if (random < 0.5) {
            // 50% chance to spawn an ice spike
            this.spikeManager.spawnSpike(x, y);
        } else {
            // 50% chance to spawn an ice phoenix
            this.phoenixManager.spawnPhoenix(x, y);
        }
        
        // No fallback to other hazards - World 3 only uses ice-themed hazards
    }
